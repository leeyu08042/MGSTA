...
parser.add_argument('--data',type=str,default='data/BJ-2-7',help='data path')
...
parser.add_argument('--checkpoint',type=str,default='modelSave/BJ-2-7.pth', help='')
...
parser.add_argument('--mixHop', type=bool, default=False, help='是否使用mixHop GCN方法')
...
def main():
    ...
    model = gwnet(device, args.num_nodes, args.dropout, supports=supports, gcn_bool=args.gcn_bool, addaptadj=args.addaptadj, aptinit=adjinit,
                   onlyEMC=args.onlyEMC, onlyNMC=args.onlyNMC, onlyADP=args.onlyADP, hops=args.hops, residual_channels=args.nhid,
                   dilation_channels=args.nhid, skip_channels=args.nhid * 8, end_channels=args.nhid * 16, mixHop=args.mixHop)
    model.to(device)
